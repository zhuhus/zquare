...
import { ZButton,ZButtonGroup } from "zquare";
import "@zquare/components/Button/style.css";
...
export const Default: Story & { args: { content: string } } = {
  ...
  render: (args: any) => ({
    ...
    template: container(
      `<z-button v-bind="args">{{args.content}}</z-button>`
    ),
  }),
  ...
  render: (args) => ({
    ...
    template: container(`
       <z-button-group :type="args.groupType" :size="args.groupSize" :disabled="args.groupDisabled">
         <z-button v-bind="args">{{args.content1}}</-button>
         <z-button v-bind="args">{{args.content2}}</-button>
       </-button-group>
    `),
  }),
  play: async ({ canvasElement, args, step }) => {
